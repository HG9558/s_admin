  ...
  <div class="member-withdraw">
   <TableSearchBar :currentPage.sync="currentPage" :searchCriteria="searchCriteria" @reloadData="getTableData" ref="searchCom">
    ...
    :showLoadTime="true"
      ...
      <!-- 表格插槽 -->
      <el-table-column :label="$t('订单号')" slot="orderNo" width="160" align="center">
      ...
      <el-table-column :label="$t('状态')" slot="reviewStatus" align="center">
        <template slot-scope="scope">
          {{[$t('不通过'), $t('通过'), $t('待审核')][scope.row.reviewStatus]}}
        </template>
      </el-table-column>
    </TableCom>
...
export default {
  ...
  mounted() {
    if (this.hasPermission("agent:commissionReview:export")) {
      this.deposit = {
        "margin-right": "75px",
      };
    }
  ...
  computed: {
    ...
    permissions() {
      ...
      return {
        edit: hasPermission("agent:commission:updateReviewStatus"),
        export: hasPermission("agent:commissionReview:export"),
      };
    },
  },
  watch: {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.$axios
        .get(systemConfig.urls.newAggentList.commissionReviewList, {
          params: {
    ...
    async handleExport() {
      ...
      const { res } = await this.$axios.GET(this.$urls.newAggentList.commissionReviewExport, params)
      if (res) {
        await this.agentCheckFile('commissionReviewModule')
      }
