<template>
  <div>
    ...
    <el-form-item :label="$t('首存类型')">
      <el-radio-group v-model="ruleScopeDtos.depositType" @change="handleConditionsChange">
        <el-radio :label="0">{{ $t('常规') }}</el-radio>
        <!-- <el-radio :label="1">{{ $t('每日首存') }}</el-radio>
        <el-radio :label="2">{{ $t('每周首存') }}</el-radio>
        <el-radio :label="3">{{ $t('限时首存') }}</el-radio> -->
      </el-radio-group>
    </el-form-item>
    <el-form-item :rules="saveTypeCheck" prop="rule.ruleScopeDtos.validDate" v-show="ruleScopeDtos.depositType == 0">
      <el-form :model="ruleScopeDtos" :rules="typeCheck" ref="saveTypeCheckCur">
        <el-form-item class="firstSaveType" prop="validDate">
          {{ $t('完成首存') }}
          <el-input type="number" v-model.number="ruleScopeDtos.validDate" @change="handleConditionsChange"></el-input>
          {{ $t('天内可领取，过期不可领取') }}
        </el-form-item>
      </el-form>
    </el-form-item>
    <el-form-item :rules="saveTypeCheck" prop="rule.ruleScopeDtos.validDate" v-show="ruleScopeDtos.depositType == 3">
      <el-form :model="ruleScopeDtos" :rules="typeCheck" ref="saveTypeCheckCur">
        <el-form-item class="firstSaveType" prop="validDate">
          {{ $t('自注册日起') }}
          <el-input type="number" v-model.number="ruleScopeDtos.day" @change="handleConditionsChange"></el-input>
          {{ $t('天内可领取，过期不可领取 （例如设定7日，则若10日13:00:00注册，16日可领，从17日00:00:00开始不可领取') }}）
        </el-form-item>
      </el-form>
    </el-form-item>
    <!-- 活动规则 -->
    <el-form-item :rules="activityRule" prop="rule.ruleScopeDtos.activityRuleDtos" :label="$t('活动规则')">
      <ActivityRuleDtosForm ref="activityRuleDtos" v-model="ruleScopeDtos.activityRuleDtos" :isMutilRuleScope="true"/>
    </el-form-item>
  </div>
</template>
